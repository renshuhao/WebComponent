...
for (function (a) {
    ...
    return "string" == typeof b && (b = { text: b }),
	    ...
	    background: "#ffffff",
	    foreground: "#000000",
	    canvasToImage: false // 图片方式显示二维码  rsh 2017-11-13
}, b),
    c = function () {
...
for (
    ...
    navigator.userAgent.toLowerCase().match(/msie ([\d.]+)/)?window.G_vmlCanvasManager.initElement(c):"",
    c.width = b.width,
    c.height = b.height,
    d = c.getContext("2d"),
    b.src && (e = new Image(),
    ...
    h < a.getModuleCount() ; h++) {
		for (i = 0; i < a.getModuleCount() ; i++) {
		    ...
		    //h == 0 ? console.log(d.fillStyle+"，x="+Math.round(i * f)+"，y="+Math.round(h * g)+"，j="+j+"，k="+k) :"",
	...
	// 如果是以canvas模式显示二维码，扩展以图片方式显示二维码  rsh 2017-11-13
	if("canvas" == b.render && (b.canvasToImage || b.renderToBackgroundImage)){
		if (b.canvasToImage && !b.renderToBackgroundImage) {
			//如果是直接canvas转img，延迟200毫秒，因为如果不延迟有可能二维码logo还没有加载完成
			setTimeout(function () {
				try{
					var image = new Image();
					image.src = e.toDataURL("image/png"); // canvas.toDataURL("image/png"),返回图片base64格式
					a(image).appendTo(_self);
				} catch(err){
					console.log(err);
					alert(err.message);
				}
			...
			// 渲染到背景图片上（背景图片和二维码图片合为一张图片） rsh 2018-01-16
			var bgImageUrl = b.renderToBackgroundImage.imageUrl;
			var img;
			var rb = document.createElement("canvas");
		    navigator.userAgent.toLowerCase().match(/msie ([\d.]+)/)?window.G_vmlCanvasManager.initElement(rb):"";
		    rb.width = b.renderToBackgroundImage.imageWidth;
		    rb.height = b.renderToBackgroundImage.imageHeight;
		    
		    var rb_d = rb.getContext("2d");
		    // 背景图片
		    if (bgImageUrl) {
		    	img = new Image(),
	    		img.src = bgImageUrl,
	    		img.crossOrigin = 'Anonymous', //开启了跨域允许，服务端响应头中附加上 Access-Control-Allow-Origin: * 字段，解决图片跨域toDataURL保存问题
	    		img.onload = function () {
	    			setTimeout(function () {
			    		rb_d.drawImage(img, 0, 0, rb.width, rb.height);
			    		// 二维码图片
			    		rb_d.drawImage(e, b.renderToBackgroundImage.x, b.renderToBackgroundImage.y, b.width, b.height)
			    		
			    		// 合成图片
			    		try{
	    					var image = new Image();
	    					image.src = rb.toDataURL("image/png"); // canvas.toDataURL("image/png"),返回图片base64格式
							a(_self).empty();
							a(image).appendTo(_self);
	    				} catch(err){
	    					console.log(err);
